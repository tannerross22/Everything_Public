...
# --- Total yearly expenditures (OpEx) ---
total_bits_current = sum(BITS_PER_YEAR.values())
total_bits_lmd = sum(results[size]["bits_lmd"] for size in BIT_SIZES)

# Current method: material + labor only (no new capital investment)
total_opex_current = total_mat_current + total_labor_current

# LMD method: material + labor + programming (machine cost is separate CapEx)
total_opex_lmd = total_mat_lmd + total_labor_lmd + annual_programming_cost

opex_difference = total_opex_current - total_opex_lmd
...
# ── YEARLY TOTAL EXPENDITURE COMPARISON ───────────────────────────────────────

print("\n" + "=" * 70)
print("  YEARLY TOTAL EXPENDITURE COMPARISON")
print("=" * 70)
print(f"\n  CURRENT METHOD (Manual):")
print(f"  ├─ Material Cost/yr:        ${total_mat_current:>12,.0f}")
print(f"  ├─ Labor Cost/yr:           ${total_labor_current:>12,.0f}")
print(f"  └─ TOTAL OpEx/yr:           ${total_opex_current:>12,.0f}")

print(f"\n  LMD METHOD (Automated):")
print(f"  ├─ Material Cost/yr:        ${total_mat_lmd:>12,.0f}")
print(f"  ├─ Labor Cost/yr:           ${total_labor_lmd:>12,.0f}")
print(f"  ├─ Programming Cost/yr:     ${annual_programming_cost:>12,.0f}")
print(f"  └─ TOTAL OpEx/yr:           ${total_opex_lmd:>12,.0f}")

print(f"\n  DIFFERENCE (Current − LMD):")
print(f"  └─ Annual Savings:          ${opex_difference:>12,.0f}")
print("=" * 70)
